...
install_key() {
    ...
    # Check and set PubkeyAuthentication to yes if it is no
    ...
    if grep -q "^PubkeyAuthentication no" "$SSHD_CONFIG" || grep -q "^#PubkeyAuthentication no" "$SSHD_CONFIG"; then
        echo -e "${INFO} Setting PubkeyAuthentication to yes..."
        if [ $(uname -o) == Android ]; then
            sed -i "s@.*\(PubkeyAuthentication \).*@\1yes@" "$SSHD_CONFIG" && {
                RESTART_SSHD=2
            ...
            $SUDO sed -i "s@.*\(PubkeyAuthentication \).*@\1yes@" "$SSHD_CONFIG" && {
                RESTART_SSHD=1
                echo -e "${INFO} PubkeyAuthentication set to yes."
            } || {
                RESTART_SSHD=0
